...
int main()
{
  ...
  // cria a memória
  mem_t *mem = init_mem();
  // cria dispositivos de E/S (o relógio, um terminal e o gerador de numeros aleatorios)
  term_t *term = term_cria();
  rel_t *rel = rel_cria();
  rand_t *rand = rand_cria(1000);
  ...
  es_registra_dispositivo(es, 3, rand, 2, rand_le, NULL);
